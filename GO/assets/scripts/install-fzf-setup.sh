...
# The repo's custom/ tree, self-located two levels up from this script
# (assets/scripts/ → repo root); the bash-era $HOME/devops-deployment
# copy stays the fallback for a lone copy of the script.
CUSTOM_SRC="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")/../.." && pwd -P)/custom/misc"
[[ -d "$CUSTOM_SRC" ]] || CUSTOM_SRC="$HOME/devops-deployment/custom/misc"
